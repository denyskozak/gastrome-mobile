...
const RecipeItemComponent = (props) => {
  ...
  const [showHint, setHintVisible] = useState(enableHint);

  useEffect(() => {
    if (showHint) setTimeout(() => setHintVisible(false), 10 * 1000);
  }, []);
      ...
      {image && (
        <TouchableHighlight style={styles.imageTouchable} activeOpacity={0.9} onPress={() => onPress(id)}>
          <View>
            ...
            {showHint && <Animated duration={800} delay={1000} outName="FadeOut" style={styles.imageIcon}>
              <Animation name="click" height={Spaces.xxxlarge * 3} width={Spaces.xxxlarge * 3}/>
